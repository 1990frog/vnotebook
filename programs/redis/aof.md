...
# 概览
Redis默认不开启。它的出现是为了弥补RDB的不足（数据的不一致性），所以它采用日志的形式来记录每个写操作，并追加到文件中。Redis重启的会根据日志文件的内容**将写指令从前到后执行一次**以完成数据的恢复工作。
## 操作粒度
aof：每条操作之后
rdb：快照时间点
# AOF配置
...
如果用户没有主动为 appendfsync 选项设置值， 那么 appendfsync 选项的**默认值为 everysec** ， 关于 appendfsync 选项的更多信息， 请参考 Redis 项目附带的示例配置文件 redis.conf 。
fsync与fdatasync控制操作系统buffer写入
## AOF 持久化的效率和安全性
