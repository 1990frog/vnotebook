...
4. 匿名内部类【重写run方法】
5. 线程池【容器管理Thread】
6. 定时器（TimerTask implements Runnable）【继承Runnable接口】
7. 通过Callable和FutureTask创建线程【Task implements Runnable】
## 总结
...
```
最终Thread thread = new Thread(futureTask);
